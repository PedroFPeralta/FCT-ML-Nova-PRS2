...
train_data_clean.loc[:, features_to_impute] = iterative_imputer.fit_transform(train_data_clean[features_to_impute])
...
test_data.loc[:, features_to_impute] = iterative_imputer.transform(test_data[features_to_impute])

final = pipeline.predict(test_data)

submission = pd.DataFrame({'id': test_data['id'], 'SurvivalTime': final})
submission.to_csv('handle-missing-submission-iteratives-01.csv', index=False)

print("\nPrevisões salvas em 'iterative-imputation-submission.csv'.")
